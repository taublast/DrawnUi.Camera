...
public partial class SkiaCamera : SkiaControl
{
    ...
    // UseRealtimeVideoProcessing moved to SkiaCamera.Processing.Shared.cs (shared across flavors)
...
#else
        // Headless / pure .NET build: no native camera hardware exists on this target.
        // Frames come from InjectFrame(), audio from InjectAudioSample() or the mock
        // capture backend wired in SkiaCamera.Net.cs.
        StartPreviewAudioCapture();
        State = HardwareState.On;
#endif
    }
...
#if IOS || ANDROID
            RescalingQuality = FilterQuality.None, //reduce power consumption
#endif
    ...
    // ProcessFrame / ProcessPreview moved to SkiaCamera.Processing.Shared.cs (shared across flavors)
    ...
    // RenderFrameForRecording / RenderPreviewForProcessing moved to SkiaCamera.Processing.Shared.cs (shared across flavors)
    ...
    {
#if !ONPLATFORM
        // Headless / pure .NET build: no OS permission system, no MainThread dispatcher to pump. Grant immediately.
        granted?.Invoke();
#else
    ...
    private void DrawDiagnosticsOverlay(SKCanvas canvas, int width, params string[] lines)
    {
        ...
        _diagFont ??= new SKFont();
        _diagFont.Size = Math.Max(14, width / 60f);
        ...
        var pad = 8f;
        var lineHeight = font.Size + 4f;
        var maxTextWidth = 0f;
        for (int i = 0; i < visibleLines.Length; i++)
        {
            maxTextWidth = Math.Max(maxTextWidth, font.MeasureText(visibleLines[i]));
        }

        var bottom = pad + visibleLines.Length * font.Size + Math.Max(0, visibleLines.Length - 1) * 4f + pad;
        ...
        for (int i = 0; i < visibleLines.Length; i++)
        {
            var y = pad + font.Size + i * lineHeight;
            canvas.DrawText(visibleLines[i], pad * 1.5f, y, font, textPaint);
        }
